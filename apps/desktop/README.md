...
The repository workbench separates Local Changes, All Commits, Pull Requests and Settings instead of rendering every feature as one page. Local Changes uses a file/diff split and a compact commit composer; All Commits keeps timeline context above the selected commit detail; Provider data remains isolated in Pull Requests.

After a successful open, Desktop remembers only the opaque repository path and structured Core launch target as a local Host preference. On the next launch it starts Core in that same environment and repeats `repository/open`, so the workbench appears without another picker. Desktop still does not inspect the path itself; an unavailable path or environment returns to the setup state with a stable error.
