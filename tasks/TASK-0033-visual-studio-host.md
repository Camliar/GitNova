...
- [ ] Visual Studio extension project and command registration
- [ ] Core lifecycle and Squash Trace interaction
- [ ] transport tests, Windows CI build gate and documentation

## Review Checklist

- [ ] Visual Studio Host 使用进程外扩展模型，不在 IDE 进程内承载业务逻辑。
- [ ] 进程使用参数 API，不经 shell；stderr 不进入协议或 UI。
- [ ] Provider 网络动作由用户显式触发并通过 Core。
- [ ] transport tests、static checks 与适用平台 build 验证通过。
