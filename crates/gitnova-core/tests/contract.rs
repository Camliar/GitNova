...
fn completes_lifecycle_and_keeps_stdout_protocol_clean() {
    ...
    assert_eq!(responses[0]["result"]["protocolVersion"], "1.14");
    ...
    assert_eq!(
        responses[0]["result"]["capabilities"]["githubPullRequestCommitDiff"],
        true
    );
    assert_eq!(
...
#[test]
fn github_pull_request_commit_diff_requires_repository_and_full_oid() {
    let oid = "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa";
    let without_repository = run(&[
        initialize(json!(1)),
        json!({"jsonrpc":"2.0","id":2,"method":"github/pullRequestCommitDiff","params":{"number":1,"oid":oid,"nameWithOwner":"owner/repo"}}),
    ]);
    let values = responses(&without_repository.stdout);
    assert_eq!(
        values[1]["error"]["data"]["stableCode"],
    ...
    let directory = TestDirectory::new("github-pr-commit-diff-params");
    git(&["init", "repo"], &directory.0);
    let repository = directory.0.join("repo");
    let output = run(&[
        initialize(json!(1)),
        repository_request(2, "repository/open", &repository),
        json!({"jsonrpc":"2.0","id":3,"method":"github/pullRequestCommitDiff","params":{"number":0,"oid":oid}}),
        json!({"jsonrpc":"2.0","id":4,"method":"github/pullRequestCommitDiff","params":{"number":1,"oid":"abc"}}),
        json!({"jsonrpc":"2.0","id":5,"method":"github/pullRequestCommitDiff","params":{"number":1,"oid":oid,"unexpected":true}}),
    ]);
    let values = responses(&output.stdout);
    assert_eq!(values[2]["error"]["code"], -32602);
    assert_eq!(values[3]["error"]["code"], -32602);
...
#[test]
fn github_squash_trace_requires_repository_and_positive_number() {
    let without_repository = run(&[
        initialize(json!(1)),
        json!({"jsonrpc":"2.0","id":2,"method":"github/squashTrace","params":{"number":1,"nameWithOwner":"owner/repo"}}),
    ]);
    let values = responses(&without_repository.stdout);
    assert_eq!(
        values[1]["error"]["data"]["stableCode"],
    ...
    let directory = TestDirectory::new("github-squash-trace-params");
    git(&["init", "repo"], &directory.0);
    let repository = directory.0.join("repo");
    let output = run(&[
        initialize(json!(1)),
        repository_request(2, "repository/open", &repository),
        json!({"jsonrpc":"2.0","id":3,"method":"github/squashTrace","params":{"number":0}}),
        json!({"jsonrpc":"2.0","id":4,"method":"github/squashTrace","params":{"number":1,"unexpected":true}}),
    ]);
    let values = responses(&output.stdout);
    assert_eq!(values[2]["error"]["code"], -32602);
    assert_eq!(values[3]["error"]["code"], -32602);
