...
> 当前仓库已包含可执行的 Core 协议、本地 Git 读模型、Core-owned GitHub/Squash Trace 能力，以及可构建的 Tauri 2 + React 19 Desktop Host。Desktop 可查看本地 status/history/diff，并显式连接 GitHub、查看 PR original commits、每个 commit 的远程文件与行级 patch，以及 Core 给出的最终 commit 关系、置信度和证据。
