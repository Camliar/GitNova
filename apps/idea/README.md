...
The project targets IntelliJ IDEA 2026.2 with Java 25 and IntelliJ Platform Gradle Plugin 2.18.1. The Java toolchain matches the target IDE's class-file level. Code instrumentation is disabled while upstream issue IDEA-390693 affects 2026.2; GitNova has no GUI Designer forms and still runs compilation, tests, and plugin packaging. Run `node scripts/check-idea.mjs` for dependency-free framing/security and toolchain checks. Run `./gradlew test buildPlugin` with a Java 25 runtime when the Gradle wrapper and IntelliJ Platform artifacts are available. Marketplace publication and automatic Core installation are out of scope.
