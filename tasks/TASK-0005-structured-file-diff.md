...
- [x] `repository/diff` method 与安全 path validation
- [x] unified patch parser 与结构化 diff 契约
- [x] Rust、JSON Schema、TypeScript 与 capability 同步
- [x] 文本、binary、空 diff、参数和仓库边界测试
- [x] Diff 协议、安全边界和刻意限制文档

## Review Checklist

- [x] Git 调用与 patch 解析只存在于 Core，不经 shell。
- [x] path 不能逃逸仓库或被解释为 pathspec magic。
- [x] hunk 范围、行类型和新旧行号通过测试。
- [x] binary 不泄露内容，Git stderr 不返回 Host。
- [x] 未实现批量/历史 diff 或任何写操作。
- [x] fmt、Clippy、Rust tests、`npm run check` 与文档检查通过。

## Done Definition

- [x] Deliverables 与 Review Checklist 完成。
- [x] 自主 Review 无阻塞意见。
- [x] 状态更新 Done，提交并推送。

## Review Record

- 2026-07-23：自主 Review 通过；补齐 rename-only path 契约、Unicode quotePath 和 CRLF 内容保真后，无剩余阻塞项。
