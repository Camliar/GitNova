...
- 新增 `gitlab/project`、`gitlab/mergeRequest`、`gitlab/mergeRequestCommitDiff`、`gitlab/squashTrace` 协议及 capability。
...
- [x] GitLab Provider and remote/project identity
- [x] Merge Request original commits, commit diff and Squash Trace
- [x] protocol/capability, errors, tests and documentation

## Review Checklist

- [x] 仅 Core 调用 `git`/`glab`，无 shell 与交互式认证。
- [x] Self-Managed hostname 来自已验证 remote，不允许 endpoint/argument 注入。
- [x] original commit membership 先验证，受限 patch 标记 unavailable，响应与数量上限失败关闭。
- [x] protocol generation、Rust tests、fmt 与 clippy 通过。
