...
> 当前仓库已包含可执行的 Core 协议、本地 Git 读模型、Core-owned GitHub/Squash Trace 能力，以及可构建的 Tauri 2 + React 19 Desktop Host。Desktop 已通过受约束的 stdio transport 管理独立 Core，可通过原生目录选择器调用 Core 打开仓库，并只读展示工作区状态；历史等后续数据切片仍由独立 Task 实现。
