...
> 当前仓库已包含可执行的 Core 协议、本地 Git 读模型、Core-owned GitHub/Squash Trace 能力，以及可构建的 Tauri 2 + React 19 Desktop Host。Desktop 已通过受约束的 stdio transport 启动和管理独立 Core；仓库选择与数据主路径将由后续独立 Task 实现。
