...
> 当前仓库已包含可执行的 Core 协议、本地 Git 读模型、Core-owned GitHub/Squash Trace 能力，以及可构建的 Tauri 2 + React 19 Desktop Host。Desktop 可打开仓库、查看本地 status/history/diff，并在用户明确触发后通过 Core 连接 GitHub、按 PR number 查看 metadata 与完整 original commits；远程 per-commit diff 和 Squash Trace 呈现仍由后续独立 Task 实现。
